...
meta_raycast(struct lua_State* L) {
	...
	int ok = raycast(ctx, &start, &over, &result, NULL, NULL);
	if (ok)
	{
		lua_pushboolean(L, 1);
...
init_meta(struct lua_State* L,int scene,struct nav_mesh_context* ctx) {
	...
	lua_newtable(L);
	lua_pushcfunction(L, meta_nav_info);
	lua_setfield(L, -2, "nav_info");
	lua_pushcfunction(L, meta_tile_info);
	lua_setfield(L, -2, "tile_info");
	lua_pushcfunction(L, meta_create_tile);
	lua_setfield(L, -2, "create_tile");
	lua_pushcfunction(L, meta_load_tile);
	lua_setfield(L, -2, "load_tile");
	lua_pushcfunction(L, meta_find);
	lua_setfield(L, -2, "find");
	lua_pushcfunction(L, meta_raycast);
	lua_setfield(L, -2, "raycast");
	lua_pushcfunction(L, meta_set_mask);
